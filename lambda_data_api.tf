...
# Install the Lambda dependencies. Only re-runs when the sources change
resource "terraform_data" "lambda_data_api" {
  triggers_replace = [
    filemd5("${path.module}/lambda_data_api/package.json"),
    filemd5("${path.module}/lambda_data_api/package-lock.json"),
    filemd5("${path.module}/lambda_data_api/index.js"),
  ]
