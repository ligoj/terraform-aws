resource aws_cognito_user_pool main {
  ...
  provisioner "local-exec" {
    when    = create
    command = "aws cognito-idp admin-create-user --user-pool-id ${aws_cognito_user_pool.main.id} --username ${var.cognito_admin} --user-attributes Name=email,Value=${var.cognito_admin} Name=email_verified,Value=true --temporary-password \"${random_password.cognito_admin.result}\""
  }
}
